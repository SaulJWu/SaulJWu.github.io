---
title: 策略
date: 2021-02-10 16:18:52
permalink: /pages/855001/
categories:
  - 后端
  - 设计模式
  - 行为型模式
...
策略模式在Java标准库应用非常广泛，我们以排序为列，看看如何通过`Arrays.sort()`实现忽略大小写排序：
...
如果我们想忽略大小写排序，就传入`String::compareToIgnoreCase`，如果我们想要倒序排序，就传入`(s1,s2) -> s1.compareTo(s2)`，这个比较两个元素大小的算法就是策略。
...
因此，上述排序使用了策略模式，它实际上指，在一个方法中，流程是确定的，但是，某些关键步骤的算法依赖调用方传入的策略，这样，传入不同的策略，即可获得不同的结果，大大增强了系统的灵活性。
