...
`StringBuilder`和`StringBuffer` 非常类似，**均代表可变的字符序列，而且提供相关功能的方法也一样**，不过它的自带的方法没有使用`synchronized`关键字，所以线程不安全而已，其他跟`StringBuffer`一模一样。所以它的底层原理跟`StringBuffer` 也是一样的。
