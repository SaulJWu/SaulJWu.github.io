---
title: InputStream
date: 2021-01-07 18:44:48
permalink: /pages/358cfe/
categories:
  - 后端
  - Java核心基础
  - IO流
...
利用上述方法一次读取多个字节时，需要先定义一个`byte[]`数组作为缓冲区，`read()`方法会尽可能多地读取字节到缓冲区， 但不会超过缓冲区的大小。
...
因为读取IO流相比执行普通代码，速度会慢很多，因此，无法确定`read()`方法调用到底要花费多长时间。
...
如果我们把代码稍微改造一下，提取一个`readAsString()`的方法：
