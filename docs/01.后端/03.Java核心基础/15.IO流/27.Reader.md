---
title: Reader
date: 2021-01-07 20:45:28
permalink: /pages/6d73ae/
categories:
  - 后端
  - Java核心基础
  - IO流
...
使用`try (resource)`结构时，当我们关闭`Reader`时，它会在内部自动调用`InputStream`的`close()`方法，所以，只需要关闭最外层的`Reader`对象即可。
 ...
 使用InputStreamReader，可以把一个InputStream转换成一个Reader。所以我们也称之为**转换流**。
...
BufferedReader顾名思义，就知道它是一个缓冲流，那么它的效率必然比FileReader的效率高，所以我们开发中应该优先使用缓冲流。前面都提过缓冲流，这里就不展开说了。
