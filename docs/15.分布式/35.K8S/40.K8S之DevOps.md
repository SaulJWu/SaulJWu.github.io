---
title: K8S之DevOps
date: 2021-08-09 13:42:29
permalink: /pages/d39bc7/
categories:
  - 分布式
  - K8S
tags:
...
![image-20210809154819007](https://gitee.com/SaulJWu/blog-images/raw/master/images/20210809154819.png)

## 3、什么是CI&CD

![file](https://gitee.com/SaulJWu/blog-images/raw/master/images/20210809153339.png)
...
- CI需要具备这些：
    ...
    - 版本控制工具。入Git,CSV,SVN等
    - 反馈机制，如构建/测试失败，可以快速地反馈到相关负责人，以尽快解决到一个更稳定的版本。
...
![file](https://gitee.com/SaulJWu/blog-images/raw/master/images/20210809153705.png)
...
[Jenkinsfile](https://www.jenkins.io/) in SCM 意为将 Jenkinsfile 文件本身作为源代码管理 (Source Control Management) 的一部分，根据该文件内的流水线配置信息快速构建工程内的 CI/CD 功能模块，比如阶段 (Stage)，步骤 (Step) 和任务 (Job)。因此，在代码仓库中应包含 Jenkinsfile。

### 目的

[本示例](https://v2-1.docs.kubesphere.io/docs/zh-CN/quick-start/devops-online/)演示如何通过 GitHub 仓库中的 Jenkinsfile 来创建流水线，流水线共包括 8 个阶段，最终将演示示例部署到 KubeSphere 集群中的开发环境和生产环境且能够通过公网访问。 仓库中的 dependency 分支为缓存测试用例，测试方式与 master 分支类似，对 dependency 的多次构建可体现出利用缓存可以有效的提升构建速度。
...
![file](http://digtime.cn/uploads/images/202009/30/1/nGg6gprXqF.png)
...
> - 阶段四. Build & push snapshot image: 根据行为策略中所选择分支来构建镜像，并将 tag 为 SNAPSHOT-BRANCHNAME−BRANCHNAME−BUILD_NUMBER推送至 Harbor (其中 $BUILD_NUMBER为 pipeline 活动列表的运行序号)。
...
在 [多租户管理快速入门](https://v2-1.docs.kubesphere.io/docs/zh-CN/quick-start/zh-CN/quick-start/admin-quick-start) 中已给项目普通用户 project-regular 授予了 maintainer 的角色，因此使用 project-regular 登录 KubeSphere，进入已创建的 devops-demo 工程，开始创建凭证。
...
![image-20210809160126072](https://gitee.com/SaulJWu/blog-images/raw/master/images/20210809160126.png)

![image-20210809160241648](https://gitee.com/SaulJWu/blog-images/raw/master/images/20210809160241.png)

![image-20210809161855874](https://gitee.com/SaulJWu/blog-images/raw/master/images/20210809161855.png)
...
![img](https://pek3b.qingstor.com/kubesphere-docs/png/sonar-id.png)
...
mvn sonar:sonar \
  ...
  -Dsonar.host.url=http://192.168.56.101:30107 \
  -Dsonar.login=5d338e1c8958926af4aa17228c53283f64f3449c
```
...
![img](https://kubesphere-docs.pek3b.qingstor.com/png/credential-list-demo.png)
...
![img](https://pek3b.qingstor.com/kubesphere-docs/png/fork-repo.png)
...
**提示：项目管理员** `project-admin`**账号已在** [多租户管理快速入门](https://v2-1.docs.kubesphere.io/docs/zh-CN/quick-start/admin-quick-start) **中创建。**
...
第一个项目创建完后，还需要项目管理员 `project-admin`邀请当前的项目普通用户 `project-regular`进入 `kubesphere-sample-dev`项目，进入「项目设置」→「项目成员」，点击「邀请成员」选择邀请 `project-regular`并授予 `operator`角色，若对此有疑问可参考 [多租户管理快速入门 - 邀请成员](https://v2-1.docs.kubesphere.io/docs/zh-CN/quick-start/admin-quick-start/#邀请成员) 。
...
![img](https://pek3b.qingstor.com/kubesphere-docs/png/WeChat6b0ca0cf57ea9c1eaf44dbac633bb459.png)
...
![img](https://pek3b.qingstor.com/kubesphere-docs/png/20190505212550.png)
...
> Webhook 推送：
>
> Webhook 是一种高效的方式可以让流水线发现远程仓库的变化并自动触发新的运行，GitHub 和 Git (如 Gitlab) 触发 Jenkins 自动扫描应该以 Webhook 为主，以上一步在 KubeSphere 设置定期扫描为辅。在本示例中，可以通过手动运行流水线，如需设置自动扫描远端分支并触发运行，详见 [设置自动触发扫描 - GitHub SCM](https://v2-1.docs.kubesphere.io/docs/v2.0/zh-CN/devops/auto-trigger)。

完成高级设置后点击 **创建**。

![img](https://pek3b.qingstor.com/kubesphere-docs/png/20190505212814.png)

### 第四步：运行流水线

流水线创建后，点击浏览器的 **刷新** 按钮，可见两条自动触发远程分支后的运行记录，分别为 `master`和 `dependency`分支的构建记录。 ![img](https://pek3b.qingstor.com/kubesphere-docs/png/WeChatb68b13f091b9e9ec52ee3ff12dd5dd8e.png)
...
![img](https://pek3b.qingstor.com/kubesphere-docs/png/sonar-result.png)
...
| :--------- | :----------------------------------------------------------- | :--------------------- | :---------------- | :------------- |
| Dev        | `http://{$Virtual IP}:{$8080}` 或者 `http://{$内网/公网 IP}:{$30861}` | kubesphere-sample-dev  | ks-sample-dev     | ks-sample-dev  |
| Production | `http://{$Virtual IP}:{$8080}` 或者 `http://{$内网/公网 IP}:{$30961}` | kubesphere-sample-prod | ks-sample         | ks-sample      |
...
> 提示：若需要在外网访问该服务，可能需要绑定公网 EIP 并配置端口转发和防火墙规则。在端口转发规则中将**内网端口**比如 30861 转发到**源端口** 30861，然后在防火墙开放这个**源端口**，保证外网流量可以通过该端口，外部才能够访问。例如在 QingCloud 云平台进行上述操作，则可以参考 [云平台配置端口转发和防火墙](https://v2-1.docs.kubesphere.io/docs/zh-CN/appendix/qingcloud-manipulation)。

至此，基于 GitHub 和 DockerHub 的一个 Jenkinsfile in SCM 类型的流水线已经完成了，若创建过程中遇到问题，可参考 [常见问题](https://v2-1.docs.kubesphere.io/docs/zh-CN/devops/devops-faq)。
...
1、点击右侧 **运行**，将根据之前设置的 **行为策略** 自动扫描代码仓库中的分支，在弹窗选择需要构建流水线的 `dependency`分支，系统将根据输入的分支加载 Jenkinsfile-online (默认是根目录下的 Jenkinsfile)。并且输入 `TAG_NAME`，点击确定。 ![img](https://pek3b.qingstor.com/kubesphere-docs/png/WeChatbdd81b896658a6958a1b315592db2306.png)

2、流水线开始运行，等待其构建完成。 ![img](https://pek3b.qingstor.com/kubesphere-docs/png/WeChatfcb5ea2d1f042a12f9120f234148ced6.png)

可发现，第二次构建利用了第一次构建时缓存的依赖，无需再次进行依赖下载。

![image-20210809201112396](https://gitee.com/SaulJWu/blog-images/raw/master/images/20210809201112.png)
